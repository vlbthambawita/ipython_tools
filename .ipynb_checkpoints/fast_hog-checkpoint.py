...
def adv_hog(image, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(3, 3),
   ...
   # from skimage import draw
    ...
    if visualize:
        return normalized_blocks, orientation_histogram,  hog_image
    else:
        return normalized_blocks

    
    
def hog_to_img(hog_vector,orientations=8, pixels_per_cell=(32, 32), image_shape = (256, 256)):
    ...
    s_row, s_col = image_shape #image.shape[:2] #
    ...
    # reshaping
    ori_hist = np.reshape(hog_vector, ( n_cells_row, n_cells_col, orientations))
    
    # compute orientations integral images
    orientation_histogram = np.zeros((n_cells_row, n_cells_col, orientations))
    orientation_histogram = ori_hist
